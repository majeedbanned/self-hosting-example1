...
class eforms extends Component {
	...
	componentDidMount() {
		...
		if (global.messageEditID != '') {
			...
			this.loadAPI(eformsID);
			this.loadAPI_content(eformsID);
		}
	}
	...
	loadAPI_content = async (eformsID) => {
		...
		let param = userInfo();
		let uurl = global.adress + '/pApi.asmx/getFormId?fId=' + eformsID + '&p=' + param + '&dmn=' + global.adress;
		console.log(uurl);
		try {
	...
	loadAPI = async (eformsID) => {
		...
		//this.setState({ loading: true });
		let param = userInfo();
		let uurl = global.adress + '/pApi.asmx/getForm?fId=' + eformsID + '&p=' + param;
		console.log(uurl);
		try {
			const response = await fetch(uurl);
			if (response.ok) {
	...
	apiPost = async (jsonstr) => {
		...
		let uurl =
			global.adress + '/pApi.asmx/setForm?p=' + param + '&json=' + jsonstr + '&formid=' + this.state.formid;
		console.log(uurl);
		try {
			const response = await fetch(uurl);
			if (response.ok) {
